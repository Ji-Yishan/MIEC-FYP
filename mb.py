        ...
        if labels is not None:
            ...
            if teacher_logits is not None:
                if weight is not None and self.strategy == "uncertainty":
                    ...
                    loss += self.kd_kl_alpha * torch.mean((1 - weight) * kd_loss, dim=0) + self.kd_rep_alpha * torch.mean(
                        weight * rep_loss, dim=0
                    )
...
class DataTrainingArguments:
    ...
    pad_to_max_length: bool = field(
        default=True,
        metadata={"help": "Pad to max_seq_length (else dynamic padding)."},
    )
...
class MBModelArguments:
    ...
    student_num_layers: Optional[int] = field(default=None, metadata={"help": "Override student layer count."})
    kl_kd: bool = field(default=False)
    temperature: float = field(default=5.0)
    objective_strategy: str = field(
        default="uncertainty",
        metadata={"help": "DynamicObjectiveKD strategy (none | uncertainty | uncertainty-r)."},
    )
...
def main():
    ...
    if data_args.task_name is not None and data_args.task_name in glue_tasks:
        try:
            ...
            if data_args.task_name == "rte" and "NoneType" in str(e):
                ...
                raise
    elif data_args.task_name is not None and data_args.task_name in task_to_keys and data_args.task_name != "sst5":
        datasets = load_dataset(data_args.task_name, cache_dir=model_args.cache_dir)
    else:
        ...
        if ext == "tsv":
            datasets = load_dataset("csv", data_files=data_files, delimiter="\t", cache_dir=model_args.cache_dir)
        elif ext == "csv":
    ...
    if data_args.task_name is not None and data_args.task_name in glue_tasks:
        ...
        lf = datasets["train"].features["label"]
        dtype = getattr(lf, "dtype", None)
        dtype_s = str(dtype) if dtype is not None else ""
        is_regression = dtype_s in ("float32", "float64") or "float" in dtype_s
        if hasattr(lf, "names") and lf.names is not None:
            is_regression = False
            label_list = list(lf.names)
            num_labels = len(label_list)
        elif is_regression:
            num_labels = 1
            label_list = []
    ...
    if data_args.task_name is not None:
        ...
        non_label = [n for n in datasets["train"].column_names if n != "label"]
        if "sentence1" in non_label and "sentence2" in non_label:
            sentence1_key, sentence2_key = "sentence1", "sentence2"
        elif len(non_label) >= 2:
            sentence1_key, sentence2_key = non_label[0], non_label[1]
        else:
            sentence1_key, sentence2_key = non_label[0], None
    ...
    if (
        ...
        ds_names = sorted(str(x).lower() for x in label_list)
        if sorted(label_name_to_id.keys()) == ds_names:
            label_to_id = {str(label_list[i]): int(label_name_to_id[str(label_list[i]).lower()]) for i in range(num_labels)}
        else:
            logger.warning("Model label2id does not match dataset; ignoring config labels.")
    elif (data_args.task_name is None and not is_regression and label_list) or data_args.task_name == "sst5":
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        if metric is not None:
            out = metric.compute(predictions=preds, references=p.label_ids)
            if len(out) > 1:
                out["combined_score"] = float(np.mean(list(out.values())))
            return out
        if is_regression:
            return {"mse": float(((preds - p.label_ids) ** 2).mean())}
    ...
    if training_args.do_predict:
        ...
        for test_split, task in zip(test_sets, tasks):
            ...
            pred = trainer.predict(test_dataset=test_split).predictions
            pred = np.squeeze(pred) if is_regression else np.argmax(pred, axis=1)
            out_f = os.path.join(training_args.output_dir, f"test_results_{task}.txt")
            if trainer.is_world_process_zero():
                os.makedirs(training_args.output_dir, exist_ok=True)
                with open(out_f, "w", encoding="utf-8") as w:
                    w.write("index\tprediction\n")
                    for i, item in enumerate(pred):
                        if is_regression:
                            w.write(f"{i}\t{item:3.3f}\n")
                        else:
                            w.write(f"{i}\t{label_list[int(item)]}\n")
