...
"""
Dynamic Knowledge Distillation – CE-KD Curriculum (prob4).
Innovation over baseline (Li et al., EMNLP 2021):
  Dynamically schedules ce_alpha (CE weight) and kd_alpha (KD weight) over training.
  Early on, high CE weight forces the student to learn directly from ground-truth labels.
  As training progresses, CE weight decreases and KD weight increases so the student
  increasingly relies on teacher soft-labels and hidden representations.
  Everything else (epoch-level segmentation, KL, PT, dynamic weighting) same as prob1.
...
class ModelArguments:
    ...
    kd_alpha: float = field(
        default=0.5,
        metadata={"help": "Base weight for KD loss (used as kd_alpha_start if curriculum not set)"}
    )
    ce_alpha: float = field(
        default=0.5,
        metadata={"help": "Base weight for Student CE loss (used as ce_alpha_start if curriculum not set)"}
    )
    ce_alpha_start: float = field(
        default=1.0,
        metadata={"help": "CE weight at training start (high = learn from labels first)"}
    )
    ce_alpha_end: float = field(
        default=0.3,
        metadata={"help": "CE weight at training end"}
    )
    kd_alpha_start: float = field(
        default=0.2,
        metadata={"help": "KD weight at training start"}
    )
    kd_alpha_end: float = field(
        default=0.8,
        metadata={"help": "KD weight at training end (high = rely on teachers)"}
    )
...
class DynamicKDModel(PreTrainedModel):
    def __init__(self, config, student, small_teacher, large_teacher, args):
        ...
        self.ce_alpha_start = args.ce_alpha_start
        self.ce_alpha_end = args.ce_alpha_end
        self.kd_alpha_start = args.kd_alpha_start
        self.kd_alpha_end = args.kd_alpha_end
        # Initialize with start values
        self.ce_alpha = args.ce_alpha_start
        self.kd_alpha = args.kd_alpha_start
    ...
    # ------------------------------------------------------------------
    # INNOVATION: CE-KD Curriculum + epoch-level segmentation
    # ------------------------------------------------------------------
    def forward(self, input_ids, attention_mask=None, label=None,
        ...
        # Curriculum: linearly interpolate ce_alpha and kd_alpha over training
        p = max(0.0, min(1.0, self._training_progress))
        self.ce_alpha = self.ce_alpha_start + p * (self.ce_alpha_end - self.ce_alpha_start)
        self.kd_alpha = self.kd_alpha_start + p * (self.kd_alpha_end - self.kd_alpha_start)
        ...
        # [安全修改 6]: 确保返回的权重 tensor 也是正确的 dtype 和 device
...
class DynamicKDTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # Set training progress (0→1) so the model can schedule CE-KD curriculum
        # [安全修改 7]: 更稳健的总步数计算，防止除零或进度失效
...
def main():
    ...
    if training_args.do_eval:
        ...
        results_path = os.path.join(training_args.output_dir, "prob4_results.json")
        os.makedirs(training_args.output_dir, exist_ok=True)
        with open(results_path, "w") as f:
            json.dump(results, f, indent=2)

        print("\n========== PROB4 RESULTS (CE-KD Curriculum) ==========")
