...
"""
Dynamic Knowledge Distillation – Attention Map Distillation (prob2).
Innovation over baseline (Li et al., EMNLP 2021):
  Adds attention map alignment loss between student and teacher attention matrices
  (as suggested by paper §3.3 referencing Jiao et al. 2020, Wang et al. 2020).
  This teaches the student HOW the teacher reasons (token-to-token relationships),
  not just what it predicts (KL) or what it represents (PT).
...
# [安全修改 1]: 尝试导入 sklearn 以替代 load_metric，兼容 Python 3.7 和各种 datasets 版本
try:
...
class ModelArguments:
    ...
    attn_alpha: float = field(
        default=0.1,
        metadata={"help": "Weight for attention map alignment loss (Innovation: Attention Distillation)"}
    )
...
class DynamicKDModel(PreTrainedModel):
    def __init__(self, config, student, small_teacher, large_teacher, args):
        ...
        self.attn_alpha = args.attn_alpha
    ...
    # ------------------------------------------------------------------
    # INNOVATION: Attention Map Distillation + epoch-level segmentation
    # ------------------------------------------------------------------
    def forward(self, input_ids, attention_mask=None, label=None,
        ...
        # Step 1: Student Forward (with attentions for attention distillation)
        ...
        # Step 4: Teacher Forward & Loss Calculation (KL + PT + Attention)
        total_kl_loss = 0.0
        total_pt_loss = 0.0
        total_attn_loss = 0.0
        count_samples = 0
        ...
        if len(small_teacher_indices) > 0:
            ...
            sub_student_attns = tuple(a[idx_tensor] for a in student_attentions)
            ...
            with torch.no_grad():
                ...
                small_t_attns = small_t_outputs.attentions
            ...
            for s_idx, t_idx in self.layer_map_small.items():
                ...
                # Attention alignment: average over heads to handle head-count mismatch
                s_attn = sub_student_attns[s_idx].mean(dim=1)  # [batch, seq, seq]
                t_attn = small_t_attns[t_idx].mean(dim=1)
        ...
        if len(large_teacher_indices) > 0:
            ...
            with torch.no_grad():
                ...
                large_t_attns = large_t_outputs.attentions
            ...
            for s_idx, t_idx in self.layer_map_large.items():
                ...
                pt_loss += F.mse_loss(s_h, t_h_proj)
                s_attn = sub_student_attns[s_idx].mean(dim=1)
                t_attn = large_t_attns[t_idx].mean(dim=1)
                attn_loss += F.mse_loss(s_attn, t_attn)
        ...
        if count_samples > 0:
            ...
            loss_attn = total_attn_loss / count_samples
        else:
            # [安全修改 3]: 使用 zeros 并匹配模型精度 (float16/float32)，防止 AMP 下类型不匹配
            loss_kl = torch.zeros((), device=device, dtype=student_logits.dtype)
            loss_pt = torch.zeros((), device=device, dtype=student_logits.dtype)
            loss_attn = torch.zeros((), device=device, dtype=student_logits.dtype)
        ...
        distill_loss = w_kl * loss_kl + w_pt * loss_pt + self.attn_alpha * loss_attn

        # Step 6: CE Loss & Total Loss
        # [安全修改 3]: 初始化 CE Loss 时也指定 dtype
        ...
        if label is not None:
            ...
            if problem_type == "regression":
                loss_fct = nn.MSELoss()
                # [安全修改 5]: 使用 view(-1) 替代 squeeze，避免 batch_size=1 时维度坍缩
        ...
        return {
            ...
            "loss_attn_raw": loss_attn.detach() if isinstance(loss_attn, torch.Tensor) else torch.zeros((), device=device, dtype=student_logits.dtype),
            "entropy": avg_norm_entropy.detach(),
            "w_kl": torch.tensor(w_kl, device=device, dtype=student_logits.dtype).detach(),
            "w_pt": torch.tensor(w_pt, device=device, dtype=student_logits.dtype).detach()
        }
...
class DynamicKDTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # Per-batch logging removed; eval is run at end of each epoch instead.
...
def main():
    ...
    # [安全修改 1]: 不再依赖 load_metric，直接使用 None，由 compute_metrics 内部处理
    metric = None 
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        # [安全修改 1]: 使用 sklearn 或手动计算，完全兼容 Python 3.7
        if is_regression:
            if HAS_SKLEARN:
    ...
    if training_args.do_eval:
        ...
        # 动态获取主指标，避免 key 错误
        score_val = eval_metrics.get("eval_accuracy", eval_metrics.get("eval_combined_score", eval_metrics.get("eval_mse", None)))
        
        results = {
            "eval_primary_score": score_val,
        ...
        results_path = os.path.join(training_args.output_dir, "prob2_results.json")
